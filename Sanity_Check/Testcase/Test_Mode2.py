...
class Test_Mode2(TestCase):
    ...
    def test_1(self):
        ...
        # 截取拍摄前患者信息
        patient_info = "/%s/current/拍摄界面截图/新建患者-1609高清-%s.png" % (self.image_path,time_str)
        ...
        if dingwei == 2:
            ...
            # 选取1609高清模式
            autoit.mouse_click("left", 1636, 244)
            time.sleep(2)
        ...
        # 按下启动拍摄按钮
        self.auto.auto_click('FOV.start_shot', times=2)

        # 判断放线时间（进度条从红色变为蓝色）
        self.ray_time=Auto_Tool.auto().Timer(x=1588,y=107,rgb_des=(34,105,148),time_delay=1,direction=0)
        print('放线时间：%s s'%self.ray_time)
        self.tf.writelines('放线时间：%s s \n'%str(self.ray_time))

        # 截取高清拍摄中预览图
        gry_current_image = "/%s/current/1609/高清拍摄中预览图%s.png" % (self.image_path,timestring)
        self.get_image.get_screenshot_by_custom_size(5, 13, 1909, 1025, gry_current_image)
        time.sleep(2)

        # 判断重建时间（从弹出dubheviewer开始计算）
        time.sleep(3)
        self.recon_time = Auto_Tool.auto().Timer(x=54, y=963, rgb_des=(255, 255, 255), time_delay=1)+3
